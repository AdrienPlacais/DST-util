...
class DSTUtilApp(tk.Tk):
    ...
    def create_distribution_tab(self) -> None:
        ...
        file_entry = ttk.Entry(distribution_frame, textvariable=self.filepath, width=40)
        file_entry.grid(row=0, column=1, padx=10, pady=10)
        file_button = ttk.Button(
            distribution_frame, text="Browse", command=self.browse_file
        )
    ...
    def browse_file(self) -> None:
        """Browse and set the file to plot."""
        filepath = filedialog.askopenfilename(
            filetypes=[("txt ASCII distribution files", "*.txt"), ("All files", "*.*")]
        )
        if filepath:
    ...
    def plot_distribution(self):
        ...
        try:
            plot_distribution(
                filepath_density=Path(filepath),
                bins=bins,
